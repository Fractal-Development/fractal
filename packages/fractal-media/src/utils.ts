...
export const getImageStyles = (image: Dimensions | null, translate: Animated.ValueXY, scale?: Animated.Value) => {
    if (!image?.width || !image?.height) {
...
export const getImageDimensionsByTranslate = (translate: Position, screen: Dimensions): Dimensions => ({
    width: screen.width - translate.x * 2,
    height: screen.height - translate.y * 2
});

export const getImageTranslateForScale = (currentTranslate: Position, targetScale: number, screen: Dimensions): Position => {
...
export const createPanResponder = ({ onGrant, onStart, onMove, onRelease, onTerminate }: PanResponderProps): PanResponderInstance =>
    PanResponder.create({
